...
import torch
import tqdm
from numpy.lib.format import _read_array_header, read_magic
...
def load_grayscale_screens(
    ...
    with zipfile.ZipFile(mdp_path, mode="r") as mdp_zip:
        with mdp_zip.open("screens.npy", mode="r") as screens_file:
            version = read_magic(screens_file)
            shape, fortran_order, dtype = _read_array_header(screens_file, version)
