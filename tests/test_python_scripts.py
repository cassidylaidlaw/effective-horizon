...
from effective_horizon.experiments.construct_tabular_policy import (
    ex as construct_tabular_policy_ex,
)
from effective_horizon.experiments.convert_atari_head_data import (
    ex as convert_atari_head_data_ex,
)
from effective_horizon.experiments.convert_atari_mdp_to_framestack import (
    ex as convert_atari_mdp_to_framestack_ex,
)
from effective_horizon.experiments.filter_to_minimal_actions import (
    ex as filter_to_minimal_actions_ex,
)


def test_convert_atari_head_data(tmp_path):
    convert_atari_head_data_ex.run(
        config_updates={
            "data_dir": "data/atari-head/freeway",
            "out_dir": str(tmp_path / "rllib_complete"),
        }
    )
...
def test_convert_atari_mdp_to_framestack(tmp_path):
    convert_atari_mdp_to_framestack_ex.run(
        config_updates={
            "mdp": "data/mdps_with_exploration_policy/freeway_10_fs30/consolidated.npz",
            "horizon": 10,
...
def test_construct_tabular_policy(tmp_path):
    ...
    for mdp_fname, checkpoint_fname, run, horizon in [
        ...
        (
            "data/mdps_with_exploration_policy/maze_easy_l0_30_fs1/consolidated.npz",
